...
import { adminDeleteUser, adminFetchAllUsers, adminFetchProfileDetails, adminUpdateUser } from '../../../store/slices/adminStore'
...
const AdminUsers = () => {
  const { user, users, error, status, productCount, filteredProductCount } = useSelector((state) => (state.AdminStore))
  ...
  useEffect(() => {
    const call = async () => {
      ...
      dispatch(adminFetchProfileDetails())
    }
    call()
  }, [dispatch])
